...
model_dir = "./saved_models/basnet_bsi1/"

epoch_num = 10000  # 训练过程中要遍历训练数据集的次数
batch_size_train = 3  # 表示训练过程中每个批次中包含的样本数量
batch_size_val = 1  # 表示验证过程中每个批次中包含的样本数量
train_num = 0  # 训练数据集样本总数
val_num = 0  # 验证数据集样本总数
...
for epoch in range(0, epoch_num):
    ...
    for i, data in enumerate(salobj_dataloader):
        ...
        # y zero the parameter gradients 参数梯度清零
        optimizer.zero_grad()
        ...
        # # print statistics  统计损失值
        running_loss += loss.item()
        running_tar_loss += loss2.item()

        # del temporary outputs and loss 删除临时输出和损失值
